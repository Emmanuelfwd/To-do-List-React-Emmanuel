...
function TaskList() {
  ...
  const cargarTareas = async () => {
    try {
      ...
      // Filtrar solo las tareas que pertenecen al usuario logueado
      const propias = lista.filter(t => t.usuarioEmail === usuario)
      setTareas(propias)
    } catch (error) {
  ...
  const agregarNuevaTarea = async () => {
    ...
    try {
      await agregarTarea({
        ...
        usuarioEmail: usuario, // Asignar la tarea al usuario actual
      })
      setNuevaTarea('')
      cargarTareas()
    } catch (error) {
